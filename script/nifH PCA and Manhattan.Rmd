...
```{r}
dfRNA <-
  readxl::read_xlsx("../data/mapping4_nifH_吉林cDNA_孟加拉-南海.xlsx")
dfDNA <-
  readxl::read_xlsx("../../吉林nifH-DNA/data/mapping3nifH吉林_赵俊.xlsx")
dfRNA <- dfRNA[dfRNA$`#SampleID` %in% 31:42,]
dfRNA$`#SampleID` <- paste0("S", dfRNA$`#SampleID`)
dfRNA$Nucleotide <- "RNA"
dfDNA <- dfDNA[dfDNA$`#SampleID` %in% paste0("S", 1:12),]
dfDNA$Nucleotide <- "DNA"
df <- rbind(dfDNA, dfRNA)

df2 <- data.frame(
  ID = df$`#SampleID`,
  Treatment = str_sub(str_split(df$Description, "_", simplify = T)[, 3], end =
                        -2),
  Nucleotide = df$Nucleotide
)
...
```{r}
otuTax <- read.csv("../table/OTUnifH.csv")
...
StrawVsCK <- ManhattanPlot("Straw","CK",1)
StrawVsCK <- ManhattanPlot("StrawN","CK",1)
